  ...
  try {
    ...
    if (typeof body.adhered !== "boolean") {
      return NextResponse.json(
        { error: "adhered must be a boolean" },
        { status: 400 },
      );
    }
    if (
      typeof body.metricValue !== "number" ||
      Number.isNaN(body.metricValue)
    ) {
      return NextResponse.json(
        { error: "metricValue must be a number" },
        { status: 400 },
      );
    }
    if (!body.date || typeof body.date !== "string") {
      return NextResponse.json(
        { error: "date must be a string (ISO or YYYY-MM-DD)" },
        { status: 400 },
      );
    }
    ...
    if (
      date <
      new Date(exp.startDate.toISOString().slice(0, 10) + "T00:00:00.000Z")
    ) {
      return NextResponse.json(
        { error: "Check-in date is before experiment startDate" },
        { status: 400 },
      );
    }
    if (
      exp.endDate &&
      date > new Date(exp.endDate.toISOString().slice(0, 10) + "T00:00:00.000Z")
    ) {
      return NextResponse.json(
        { error: "Check-in date is after experiment endDate" },
        { status: 400 },
      );
    }
